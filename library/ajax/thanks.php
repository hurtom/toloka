...
switch ($mode) {
    case 'thanks':
        ...
        if (!isset($row['user_id'])) {
            DB()->query("UPDATE " . BB_ATTACHMENTS_DESC . " SET thanks_count = thanks_count + 1 WHERE attach_id = $attach_id");
    ...
    case 'list':
        $sql = "SELECT u.user_id, u.username
            ...
            AND thanked = 1";
        $users_thanked = DB()->fetch_rowset($sql);
        foreach($users_thanked as $row) {
